...
#             x_pick.append( x[n] )
#         else:
#             if x_change_known and x[n] >= last_big_known and x[n] > x_change_known[0]:
#                 x_change_known.append( x[n] )
#                 x_unknown.append( 'none' )
#                 x_pick.append( x[n] )
#             else:
...
def data_processor( case, unpackaged_useful_elements ):
    ...
    for var in range( len( vars_as_appear ) ):
        ...
        for k in range( len( this_var_dict[this_variable] ) ):
                ...
                if this_tech in list( Fleet_Groups_inv.keys() ):
                    ...
                    if 'Trains' not in group_tech and 'Telef' not in group_tech:
                        ...
                        driven_distance_2024 = \
                            float(Reference_driven_distance[this_strategy][group_tech][time_range_vector.index(2025)])
                        driven_distance_final = \
                            float(Reference_driven_distance[this_strategy][group_tech][time_range_vector.index(2055)])
                        ...
                        if this_variable == 'NewCapacity':
                            ...
                            if int(this_year) >= 2025 and driven_distance_2024 != driven_distance_final:
                                # We need to select the appropiate accumulated new capacity:
                                accum_new_cap = \
                                    aide_dict_accumnewcap[this_tech][this_year_index-1]

                                if int(this_year) > 2021+Reference_op_life[this_strategy][this_tech]:  # works with a timeframe by 2050; longer timeframes may need more lifetime cycle adjustments
                                    subtract_index_accum_old = \
                                        int(Reference_op_life[this_strategy][this_tech])
                                    if int(this_year) - subtract_index_accum_old <= 2025:
                                        driven_distance_17 = \
                                            float(Reference_driven_distance[this_strategy][group_tech][time_range_vector.index(2022)])
                                    else:
                                        driven_distance_17 = \
                                    ...
                                    if int(this_year) - subtract_index_accum_old > 2025:  # this is an odd rule, but works empirically
                                        driven_distance_2024 = \
                                            float(Reference_driven_distance[this_strategy][group_tech][time_range_vector.index(2025)])
            ...
            #
            if this_combination[2] in ['Accidents', 'Health', 'Congestion', 'CONVAR', 'CONHICK', 'REHAB'] and this_variable == 'AnnualTechnologyEmissionPenaltyByEmission':
                ref_index = combination_list.index( this_combination )
                this_data_row = deepcopy( data_row_list[ ref_index ] ) # this must be updated in a further position of the list
                #
            ...
            if this_variable == 'CapitalInvestment':
                ...
                if this_tech in list(Fleet_Groups_inv.keys()):
                    ...
                    if 'Trains' not in group_tech and 'Telef' not in group_tech:
                        ...
                        accum_new_cap_subtract = 0
                        driven_distance_prev = float(Reference_driven_distance[this_strategy][group_tech][this_year_index-1])
                        if int(this_year) >= 2025 and driven_distance_2024 != driven_distance_final:  # greater than the base year
                            # We need to select the appropiate accumulated new capacity:
                            accum_new_cap = \
                                aide_dict_accumnewcap[this_tech][this_year_index-1]

                            if int(this_year) > 2021+Reference_op_life[this_strategy][this_tech]:
                                subtract_index_accum_old = \
                                    int(Reference_op_life[this_strategy][this_tech])
                                if int(this_year) - subtract_index_accum_old <= 2025:
                                    driven_distance_17 = \
...
#            f.write( 'param ' + this_param + ':=\n' )
    ...
    #g.write('param VariableCost default 0 :=\n;\n')
    g.write('param AvailabilityFactor default 1 :=\n;\n')
    # if scenario_list[scen] == 'BAU':  # This is commented if no electrical scenario features are added
...
if __name__ == '__main__':
    #
    all_years = [ y for y in range( 2021 , 2055+1 ) ]
    index_2024 = all_years.index( 2025 )
    ...
    *Abbreviations:*
    ...
    for t in range( len( list( Fleet_Groups_techs_2_dem.keys() ) ) ):
        ...
        if 'MAR' in this_fuel:
            group_tech_MARITIME.append( tech_key )
        if 'FREMAM' in this_fuel:
            group_tech_FREIGHT_MAM.append( tech_key )    
        if 'AVI' in this_fuel:
            group_tech_AVIATION.append( tech_key )    
    #
    group_tech_PASSENGER = group_tech_PUBLIC + group_tech_PRIVATE + group_tech_MARITIME + group_tech_AVIATION
    group_tech_FREIGHT = group_tech_FREIGHT_HEA + group_tech_FREIGHT_LIG + group_tech_FREIGHT_MAM
    #
    ...
    base_configuration_modeshift2 = pd.read_excel('B1_Scenario_Config.xlsx', sheet_name='Mode_Shift2')
    #
    ...
    for s in range( len( scenario_list ) ):
        ...
        passmar_range_indices = [ i for i, x in enumerate( stable_scenarios[ scenario_list[s] ][ 'SpecifiedAnnualDemand' ][ this_set_type_initial ] ) if x == str( 'E6TDPASMAR' ) ]
        passavi_range_indices = [ i for i, x in enumerate( stable_scenarios[ scenario_list[s] ][ 'SpecifiedAnnualDemand' ][ this_set_type_initial ] ) if x == str( 'E6TDPASAVI' ) ]
        ...
        mfre_range_indices = [ i for i, x in enumerate( stable_scenarios[ scenario_list[s] ][ 'SpecifiedAnnualDemand' ][ this_set_type_initial ] ) if x == str( 'E6TDFREMAM' ) ]
        #
        ...
        if scenario_list[s] == 'BAU':
            ...
            passmar_values = deepcopy( stable_scenarios[ scenario_list[s] ][ 'SpecifiedAnnualDemand' ]['value'][ passmar_range_indices[0]:passmar_range_indices[-1]+1 ] )
            passavi_values = deepcopy( stable_scenarios[ scenario_list[s] ][ 'SpecifiedAnnualDemand' ]['value'][ passavi_range_indices[0]:passavi_range_indices[-1]+1 ] )
            ...
            ref_fre_mam = deepcopy( stable_scenarios[ scenario_list[s] ][ 'SpecifiedAnnualDemand' ]['value'][ mfre_range_indices[0]:mfre_range_indices[-1]+1 ] )
            #
            Total_Demand = []
            Total_Demand_Fre = []
            for n in range( len( time_range_vector ) ):
                # Total_Demand.append( float( passpub_values[n] ) + float( passpriv_values[n] ) + float( passmar_values[n] ) + float( passavi_values[n] ) + float ( nonmotorized_values[n] ) )
                Total_Demand.append( float( passpub_values[n] ) + float( passpriv_values[n] ) + float ( nonmotorized_values[n] ) )
                # Total_Demand_Fre.append( float( ref_fre_hea[n] ) + float( ref_fre_lig[n] ) + float( ref_fre_mam[n] ) )
            ...
            ref_pass_mar_shares    = [ float(passmar_values[n])/Total_Demand[n] for n in range( len( time_range_vector ) ) ]
            ref_pass_avi_shares    = [ float(passavi_values[n])/Total_Demand[n] for n in range( len( time_range_vector ) ) ]
        ...
        #
            ...
            for a_set in range( len( applicable_sets_demand ) ): # FLAG: in the future we must consider non-logistic behaviors
                ...
                if modeshift_params[ scenario_list[s] ][ this_set ][ 'Type' ] == 'Logistic':
                    ...
                    r2050 = 1/R2050
                    Q =     L/C - 1
                    k =     np.log( (r2050-1)/Q )/(M-2055)
                    #
                    shift_years = [ n for n in range( Initial_Year_of_Uncertainty+1,2055+1 ) ]
        ...
        for capvar in range( len( capacity_variables ) ):
            ...
            for g in range( len( group_tech_PUBLIC ) ):
                ...
                #
            #----------------------------------------------------------------------------------------------------------------------------------------------------------
            for g in range( len( group_tech_MARITIME ) ):
                #
                cap_indices = [ i for i, x in enumerate( stable_scenarios[ scenario_list[s] ][ capacity_variables[capvar] ][ this_set_type_initial ] ) if x == str( group_tech_MARITIME[g] ) ]
                or_indices = [ i for i, x in enumerate( stable_scenarios[ scenario_list[s] ][ 'OutputActivityRatio' ][ this_set_type_initial ] ) if x == str( group_tech_MARITIME[g] ) ]
                demand_indices = [ i for i, x in enumerate( stable_scenarios[ scenario_list[s] ][ 'SpecifiedAnnualDemand' ][ 'f' ] ) if x == str( Fleet_Groups_techs_2_dem[ group_tech_MARITIME[g] ] ) ]
                #
                # here we quickly call the occupancy rates
                or_values = deepcopy( stable_scenarios[ scenario_list[s] ][ 'OutputActivityRatio' ]['value'][ or_indices[0]:or_indices[-1]+1 ] )
                or_values = [ float( or_values[j] ) for j in range( len( or_values ) ) ]
                ...
                if scenario_list[s] == 'BAU': # REMEMBER THE VARIABLES OF BAU DO NOT CHANGE
                    ...
                    this_pkm_share_rounded = [ round(elem, 4) for elem in this_pkm_share ]
                    #
                    relative_pkm_to_demand[ capacity_variables[capvar] ].update( { group_tech_MARITIME[g]:this_pkm_share_rounded } )
                #
                else: # if scenario_list[s] in ['NDP','NDP_A','OP15C']: # we must apply the same distribution of transport in pkm as in the BAU, considering the new demands. The variable to modify is the capacity.
                    #
                    apply_these_shares = relative_pkm_to_demand[ capacity_variables[capvar] ][ group_tech_MARITIME[g] ]
                    new_cap_values = [ apply_these_shares[n]*demand_values[n]/or_values[n] for n in range( len( time_range_vector ) ) ]
                    new_cap_values_rounded = [ round(elem, 4) for elem in new_cap_values ]
                    #
                    stable_scenarios[ scenario_list[s] ][ capacity_variables[capvar] ]['value'][ cap_indices[0]:cap_indices[-1]+1 ] = deepcopy( new_cap_values_rounded )
                ...
                #
            #----------------------------------------------------------------------------------------------------------------------------------------------------------
            for g in range( len( group_tech_AVIATION ) ):
                #
                cap_indices = [ i for i, x in enumerate( stable_scenarios[ scenario_list[s] ][ capacity_variables[capvar] ][ this_set_type_initial ] ) if x == str( group_tech_AVIATION[g] ) ]
                or_indices = [ i for i, x in enumerate( stable_scenarios[ scenario_list[s] ][ 'OutputActivityRatio' ][ this_set_type_initial ] ) if x == str( group_tech_AVIATION[g] ) ]
                demand_indices = [ i for i, x in enumerate( stable_scenarios[ scenario_list[s] ][ 'SpecifiedAnnualDemand' ][ 'f' ] ) if x == str( Fleet_Groups_techs_2_dem[ group_tech_AVIATION[g] ] ) ]
                #
                # here we quickly call the occupancy rates
                or_values = deepcopy( stable_scenarios[ scenario_list[s] ][ 'OutputActivityRatio' ]['value'][ or_indices[0]:or_indices[-1]+1 ] )
                or_values = [ float( or_values[j] ) for j in range( len( or_values ) ) ]
                ...
                if scenario_list[s] == 'BAU': # REMEMBER THE VARIABLES OF BAU DO NOT CHANGE
                    ...
                    this_pkm_share_rounded = [ round(elem, 4) for elem in this_pkm_share ]
                    #
                    relative_pkm_to_demand[ capacity_variables[capvar] ].update( { group_tech_AVIATION[g]:this_pkm_share_rounded } )
                #
                else: # if scenario_list[s] in ['NDP','NDP_A','OP15C']: # we must apply the same distribution of transport in pkm as in the BAU, considering the new demands. The variable to modify is the capacity.
                    #
                    apply_these_shares = relative_pkm_to_demand[ capacity_variables[capvar] ][ group_tech_AVIATION[g] ]
                    new_cap_values = [ apply_these_shares[n]*demand_values[n]/or_values[n] for n in range( len( time_range_vector ) ) ]
                    new_cap_values_rounded = [ round(elem, 4) for elem in new_cap_values ]
                    #
                    stable_scenarios[ scenario_list[s] ][ capacity_variables[capvar] ]['value'][ cap_indices[0]:cap_indices[-1]+1 ] = deepcopy( new_cap_values_rounded )
        ...
        ###        Sets_Involved = ['Techs_He_Freight', 'Techs_Li_Freight', 'Techs_Trains_Freight', 'Techs_SUV', 'Techs_Auto', 'Techs_Motos', 'Techs_Buses_Pub', 'Techs_Buses_Pri', 'Techs_Buses_Micro', 'Techs_Taxi']
        ...
        ###    zzz1 = list(set(list(stable_scenarios['NDP']['TotalAnnualMaxCapacity']['t'])))
        ###    zzz1b = list(set(list(stable_scenarios['NDP']['TotalAnnualMinCapacity']['t'])))
        ###    print('zzz1', len(zzz1), len(zzz1b))
        ###   #  sys.exit()
        ...
        rewrite_techs_maxcap = \
            [e for e in unique_max_cap_techs if
             'TR' in e and 'IN' not in e and 'LU' not in e and e not in ignore_techs]
        rewrite_techs_lowlim = \
            [e for e in unique_low_lim_techs if
             'TR' in e and 'IN' not in e and 'IMP' not in e and 'LU' not in e and e not in ignore_techs]
        ...
        # Mode_Shift2
        if scenario_list[s] in list(set(base_configuration_modeshift2['Scenario'].tolist())):            
            ''' MODIFYING *base_configuration_modeshift2* '''
            this_scenario_df = deepcopy(base_configuration_modeshift2.loc[base_configuration_modeshift2['Scenario'].isin([scenario_list[s]])])
        ...
        if scenario_list[s] in list( set( list( adoption_params.keys() ) ) ):
            ...
            for a_set in range( len( applicable_sets ) ):
                ...
                if this_type == 'Linear':
                    ...
                    #
                    known_years_ini = [e for e in range(2021, int(y_ini)+1)]
                ...
                for cp in range( len( cap_vars ) ):
                    ...
                    if len( this_set_range_indices ) != 0:
                        ...
                        # print('happens??', this_set, this_sector, scenario_list[s], cap_vars[cp])
                        for y in range( len( time_range_vector ) ):
                            stable_scenarios[ scenario_list[s] ][ cap_vars[ cp ] ]['r'].append( 'JAM' )
        ...
        if scenario_list[s] != 'BAU': # we proceed to adjust first the parameters and corresponding sets affected by distance, then the demands.
            for p in range(len(parameters_involved)):
                ...
                if p == 0:
                    ...
                    dem_adj_maritime = True
                    dem_adj_aviation = True
                    dem_adj_heafre = True
                    dem_adj_ligfre = True
                    dem_adj_mamfre = True
                ...
                for a_set in range( len( transport_group_sets ) ):
                    ...
                    if ((this_demand_set == 'E6TDPASPRI' and dem_adj_private is True) or
                        ...
                        (this_demand_set == 'E6TDPASMAR' and dem_adj_maritime is True) or
                        (this_demand_set == 'E6TDPASAVI' and dem_adj_aviation is True) or
                        (this_demand_set == 'E6TDFREHEA' and dem_adj_heafre is True) or
                        (this_demand_set == 'E6TDFRELIG' and dem_adj_ligfre is True) or
                        (this_demand_set == 'E6TDFREMAM' and dem_adj_mamfre is True)):
                        ...
                        if Fleet_Groups_techs_2_dem[ this_group_set ] == 'E6TDPASPUB' and scenario_list[ s ] != 'BAU':
                            subtract_list = [float(train_pass_capacity_values[j]) for j in range(len(train_pass_capacity_values))]
                        else:
                            subtract_list = [0 for j in range(len(train_pass_capacity_values))]
                        ...
                        if this_demand_set == 'E6TDPASPUB':
                            ...
                            #
                        if this_demand_set == 'E6TDPASMAR':
                            dem_adj_maritime = False
                            #
                        if this_demand_set == 'E6TDPASAVI':
                            dem_adj_aviation = False
                            #
                        ...
                        if this_demand_set == 'E6TDFRELIG':
                            ...
                            #
                        if this_demand_set == 'E6TDFREMAM':
                            dem_adj_mamfre = False
        ...
        for g in range( len( transport_group_sets ) ): # this actually avoids the maxcap issue
            ...
            for t in range( len( this_tech_set_list ) ):
                ...
                if this_tech in unique_residual_cap_techs and this_tech not in unique_max_cap_techs:
                    ...
                    #
                    for y in range( len( time_range_vector ) ):
                        stable_scenarios[ scenario_list[s] ][ 'TotalAnnualMaxCapacity' ]['r'].append( 'JAM' )
                ...
                for p in range( len( param_list ) ):
                    ...
                    if electrical_Params_built_in == 'NO':
                        ...
                        if 'Write' in electrical_Params_methods:
                            ...
                            for y in range( len( time_range_vector ) ):
                                stable_scenarios[ this_scenario_name ][ param_list[p] ]['r'].append( 'JAM' )
            ...
            for t in range( len( tech_list ) ):
                ...
                for p in range( len( param_list ) ):
                    ...
                    if smartGrid_Params_built_in == 'YES':
                        ...
                        if param_list[p] == 'EmissionActivityRatio': #len(this_param_indices) > len(time_range_vector):
