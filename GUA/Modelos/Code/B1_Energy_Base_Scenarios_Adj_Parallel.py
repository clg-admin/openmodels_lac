...
def main_executer(n1, packaged_useful_elements, scenario_list_print, discount_year, discount_rate):
    ...
    file_aboslute_address = os.path.abspath("B1_Energy_Base_Scenarios_Adj_Parallel")
    file_adress = re.escape( file_aboslute_address.replace( 'B1_Energy_Base_Scenarios_Adj_Parallel', '' ) ).replace( '\:', ':' ).replace(r'Code\\', '')
    file_adress+=r'M3_Energy\\'
    #
...
def set_first_list(scenario_list_print):
    #
    first_list_raw = os.listdir( '../M3_Energy/Executables' )
    #
    global first_list
...
def data_processor( case, unpackaged_useful_elements, discount_year, discount_rate ):
    ...
    df_fuel_to_code = pd.read_excel( '../M3_Energy/A1_Inputs/A-I_Classifier_Modes_Transport.xlsx', sheet_name='Fuel_to_Code' )
    df_fuel_2_code_fuel_list        = df_fuel_to_code['Code'].tolist()
    df_fuel_2_code_plain_english    = df_fuel_to_code['Plain_English'].tolist()
    df_tech_to_code = pd.read_excel( '../M3_Energy/A1_Inputs/A-I_Classifier_Modes_Transport.xlsx', sheet_name='Tech_to_Code' )
    ...
    #-------------------------------------------#
    structure_filename = "../M3_Energy/B1_Model_Structure.xlsx"
    ...
    filter_vars = [ 'FilterFuelType',
                    ...
                    #
                    'Capex'+str(discount_year), # CapitalInvestment
    ...
    #
    vars_as_appear = []
    data_name = str( '../M3_Energy/Executables/' + first_list[case] ) + '/' + str(first_list[case]) + '_output.txt'
    #
    n = 0
    ...
    #-----------------------------------------------------------------------------------------------------------%
    output_adress = '../M3_Energy/Executables/' + str( first_list[case] )
...
#            f.write( 'param ' + this_param + ':=\n' )
    ...
    ###########################################################################################################################
    #
    with open( '../M3_Energy/Executables' + '/' + str( scenario_list[scen] ) + '_0' + '/' + str( scenario_list[scen] ) + '_0' + '_Input.csv', 'w', newline = '') as param_csv:
...
def interpolation_blend( start_blend_point, mid_blend_point, final_blend_point, value_list, time_range_vector ):
    ...
    #
#
#TODO 5/6 Added new interpolation function by Edgar Miranda
def new_blend_2024(start_blend_point, mid_blend_point_one, mid_blend_point_two,final_blend_point, value_list, time_range_vector,name):
    """
    This function implement ethanol to gasoline vehicles with the posibilitie
    of three blends years.

    Parameters:
        start_blend_point : set the initial year
        min_blend_point_one : set the first blend year
        min_blend_point_two : set the second blend year
        final_blend_point : set the final year
        value_list : list of gasoline tech emissions
        time_range_vector : time range vector
        name : name of the technology to be blended

        Retuns
        the new values of the gasoline tech emissions and the biofuel shares
        If needed a csv file can be generate with the value just uncomment the line 1613 - 1617
    """
    name = name
    years = np.array([start_blend_point[0], mid_blend_point_one[0], mid_blend_point_two[0],
                      final_blend_point[0]])
    values = np.array([start_blend_point[1] / 100, mid_blend_point_one[1] / 100, mid_blend_point_two[1] / 100,
                        final_blend_point[1] / 100])

    new_years = np.arange(time_range_vector[0], time_range_vector[-1] + 1)
    interpolated_values = np.zeros_like(new_years, dtype=float)

    for i, year in enumerate(new_years):
        index_year = np.searchsorted(years, year, side='right') - 1
        interpolated_values[i] = values[index_year]

    interpolated_values = interpolated_values.tolist()

    # Multiply to the gasoline tech emissions
    new_value_list = []
    for n in range(len(value_list)):
        new_value_list.append(value_list[n] * (1 - interpolated_values[n]))
    new_value_list_rounded = [round(elem, 4) for elem in new_value_list]
    biofuel_shares = [round(elem, 4) for elem in interpolated_values]
    #
    # Convert a csv file if needed
    # dataframe = pd.DataFrame({'Year': new_years,
    #                           'Emissiones_gasoline': value_list,
    #                           'Blend_inter_value': biofuel_shares,
    #                           'New_emissiones_gasoline': new_value_list_rounded})
    # dataframe.to_csv(f'Imagenes+diagrama/Blend_values_new{name}.csv', index=False)
...
def csv_printer_parallel(s, scenario_list, stable_scenarios, basic_header_elements, parameters_to_print, S_DICT_params_structure):
    ...
    for p in range( len( parameters_to_print ) ):
        param_filename = '../M3_Energy/B1_Output_Params/' + str( scenario_list[s] ) + '/' + str( parameters_to_print[p] ) + '.csv'
...
def run_model(discount_year, discount_rate):
    ...
    *Abbreviations:*
    ...
    start1 = time.time()
    #
    # Added by Edgar Miranda 2024
    """Run to modify the InputActivityRatio.csv file (Efficiency in cars)"""
    os.system('python Efficiency_inputAR.py')
    
    # We must open useful GDP data for denominator
    df_gdp_ref = pd.read_excel('../M3_Energy/_GDP_Ref.xlsx', 'GDP')
    ...
    '''
    Fleet_Groups =              pickle.load( open( '../M3_Energy/A1_Outputs/A-O_Fleet_Groups.pickle',            "rb" ) )
    ### Ind_Groups =                pickle.load( open( '../M3_Energy/A1_Outputs/A-O_Ind_Groups.pickle',            "rb" ) )
    ### Fleet_Groups['Techs_Microbuses'] += [ 'TRMBUSHYD' ] # this is an add on, kind of a patch
    Fleet_Groups_Distance =     pickle.load( open( '../M3_Energy/A1_Outputs/A-O_Fleet_Groups_Distance.pickle',   "rb" ) )
    Fleet_Groups_OR =           pickle.load( open( '../M3_Energy/A1_Outputs/A-O_Fleet_Groups_OR.pickle',         "rb" ) )
    Fleet_Groups_techs_2_dem =  pickle.load( open( '../M3_Energy/A1_Outputs/A-O_Fleet_Groups_T2D.pickle',        "rb" ) )
    #
    Fleet_Groups_inv = {}
    ...
    #
    scenario_list_sheet = pd.read_excel( '../M3_Energy/B1_Scenario_Config.xlsx', sheet_name='Scenarios' )
    ...
    #sys.exit()
    #
    base_configuration_overall = pd.read_excel( '../M3_Energy/B1_Scenario_Config.xlsx', sheet_name='Overall_Parameters' )
    ### base_configuration_transport_elasticity = pd.read_excel( '../M3_Energy/B1_Scenario_Config.xlsx', sheet_name='TElasticity' )
    base_configuration_distance = pd.read_excel( '../M3_Energy/B1_Scenario_Config.xlsx', sheet_name='Distance_Levers' )
    base_configuration_modeshift = pd.read_excel( '../M3_Energy/B1_Scenario_Config.xlsx', sheet_name='Mode_Shift' )
    base_configuration_or = pd.read_excel( '../M3_Energy/B1_Scenario_Config.xlsx', sheet_name='Occupancy_Rate' )
    base_configuration_adoption = pd.read_excel( '../M3_Energy/B1_Scenario_Config.xlsx', sheet_name='Tech_Adoption' )
    base_configuration_electrical = pd.read_excel( '../M3_Energy/B1_Scenario_Config.xlsx', sheet_name='Electrical' )
    base_configuration_smartGrid = pd.read_excel( '../M3_Energy/B1_Scenario_Config.xlsx', sheet_name='SmartGrid' )
    base_configuration_E_and_D = pd.read_excel( '../M3_Energy/B1_Scenario_Config.xlsx', sheet_name='Efficiency' )
    #
    all_dataset_address = '../M3_Energy/A2_Output_Params/'
    '''
    # Call the default parameters for later use:
    '''
    list_param_default_value = pd.read_excel( '../M3_Energy/B1_Default_Param.xlsx' )
    ...
    #
    for scen in range( len( scenario_list ) ):
        this_paramter_list_dir = '../M3_Energy/A2_Output_Params/' + str( scenario_list[scen] )
            ...
            for a_set in range( len( applicable_sets ) ):
                ...
                for cp in range( len( cap_vars ) ):
                    ...
                    if len( this_set_range_indices ) != 0:
                        ...
                        # print('happens??', this_set, this_sector, scenario_list[s], cap_vars[cp])
                        for y in range( len( time_range_vector ) ):
                            stable_scenarios[ scenario_list[s] ][ cap_vars[ cp ] ]['r'].append( 'GUA' )
        ...
        for g in range( len( transport_group_sets ) ): # this actually avoids the maxcap issue
            ...
            for t in range( len( this_tech_set_list ) ):
                ...
                if this_tech in unique_residual_cap_techs and this_tech not in unique_max_cap_techs:
                    ...
                    #
                    for y in range( len( time_range_vector ) ):
                        stable_scenarios[ scenario_list[s] ][ 'TotalAnnualMaxCapacity' ]['r'].append( 'GUA' )
        ...
        if scenario_list[s] != 'BAU': # GENERALIZE THIS LATER.
            ...
            for n in range( len( Diesel_Techs ) ):
                ...
                for n2 in range( len( Diesel_Techs_Emissions[ Diesel_Techs[n] ] ) ):
                    ...
                    value_list = [ float(value_list[j]) for j in range( len(value_list) ) ]
                    #
                    start_blend_point = [2018, 0]
                    mid_blend_point_one = [2026, 10]
                    mid_blend_point_two = [2030, 15]
                    final_blend_point = [2040, 20]


                    new_value_list_rounded, biofuel_shares = new_blend_2024(start_blend_point, mid_blend_point_one, mid_blend_point_two, final_blend_point,
                                                                            value_list, time_range_vector,Gasoline_Techs[n])
                    #
            ...
            for n in range( len( Gasoline_Techs ) ):
                ...
                for n2 in range( len( Gasoline_Techs_Emissions[ Gasoline_Techs[n] ] ) ):
                    ...
                    value_list = [ float(value_list[j]) for j in range( len(value_list) ) ]
                    #
                    start_blend_point = [2024, 0]
                    mid_blend_point = [2025, 5]
    ...
    ###if scenario_list[s] == 'NDP':
    ###    zzz2 = list(set(list(stable_scenarios['NDP']['TotalAnnualMaxCapacity']['t'])))
    ###    zzz2b = list(set(list(stable_scenarios['NDP']['TotalAnnualMinCapacity']['t'])))
    ###    print('zzz2', len(zzz2), len(zzz2b))
    ###    #sys.exit()
    ...
    #
    with open('../M3_Energy/Blend_Shares_0.pickle', 'wb') as handle:
                ...
                for p in range( len( param_list ) ):
                    ...
                    if electrical_Params_built_in == 'NO':
                        ...
                        if 'Write' in electrical_Params_methods:
                            ...
                            for y in range( len( time_range_vector ) ):
                                stable_scenarios[ this_scenario_name ][ param_list[p] ]['r'].append( 'GUA' )
    ...
    ###if scenario_list[s] == 'NDP':
    ###    zzz3 = list(set(list(stable_scenarios['NDP']['TotalAnnualMaxCapacity']['t'])))
    ###    zzz3b = list(set(list(stable_scenarios['NDP']['TotalAnnualMinCapacity']['t'])))
    ###    print('zzz3', len(zzz3), len(zzz3b))
    ###    #sys.exit()
    ...
    if generator_or_executor == 'Generator' or generator_or_executor == 'Both':
        ...
        #
        print_adress = '../M3_Energy/Executables'
        #
...
def main():
    ...
    print("*********************************************")
    print("9. Corriendo el modelo del sector Energia ...")
    print("**********************************************")
