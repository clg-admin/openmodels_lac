...
def main_executer(n1, packaged_useful_elements, scenario_list_print, discount_year, discount_rate):
    ...
    file_aboslute_address = os.path.abspath("B1_IPPU_EjecucionOSeMOSYS.py")
    file_adress = re.escape(file_aboslute_address.replace('B1_IPPU_EjecucionOSeMOSYS.py', '')).replace('\:', ':').replace(r'Code\\', '')
    file_adress+=r'M0_IPPU\\'
    #
...
def set_first_list(scenario_list_print):
    #
    first_list_raw = os.listdir('../M0_IPPU/Executables')
    #
    global first_list
...
def data_processor(case, unpackaged_useful_elements, discount_year, discount_rate):
    ...
    #-------------------------------------------#
    structure_filename = "../M0_IPPU/B1_Model_Structure.xlsx"
...
#        output_header.append(more_vars[v])
    ...
    data_name = str('../M0_IPPU/Executables/' + first_list[case]) + '/' + str(first_list[case]) + '_output.txt'
    #
    n = 0
    ...
    #%%
    #-----------------------------------------------------------------------------------------------------------%
    output_adress = '../M0_IPPU/Executables/' + str(first_list[case])
    ...
    for var in range(len(vars_as_appear)):
        ...
        for k in range(len(this_var_dict[this_variable])):
            ...
            #
            if this_combination[2] in ['CO2e_CEM','CO2e_CAL','CO2e_VID','CO2e_CARBONATO','CO2e_HIER','CO2e_FERRO','CO2e_LUB','CO2e_CERAS','CO2e_RAC','RAC_COST'] and this_variable == 'AnnualTechnologyEmissionPenaltyByEmission':
                ref_index = combination_list.index(this_combination)
...
#            f.write('param ' + this_param + ':=\n')
    ...
    ###########################################################################################################################
    #
    with open('../M0_IPPU/Executables' + '/' + str(scenario_list[scen]) + '_0' + '/' + str(scenario_list[scen]) + '_0' + '_Input.csv', 'w', newline = '') as param_csv:
...
def run_model(discount_year, discount_rate):
    ...
    # Year periods:
    dfyrs = \
        pd.read_excel('../M0_IPPU/A1_Inputs/A-I_Horizon_Configuration.xlsx',
                      sheet_name='Horizon')
    ...
    *Abbreviations:*
    ...
    df_gdp_ref = pd.read_excel('../M0_IPPU/_GDP_Ref.xlsx', 'GDP')
    ...
    #
    scenario_list_sheet = pd.read_excel('../M0_IPPU/B1_Scenario_Config.xlsx', sheet_name='Scenarios')
    ...
    #sys.exit()
    #
    base_configuration_overall = pd.read_excel('../M0_IPPU/B1_Scenario_Config.xlsx', sheet_name='Overall_Parameters')
    #
    all_dataset_address = '../M0_IPPU/A2_Output_Params/'
    '''
    # Call the default parameters for later use:
    '''
    list_param_default_value = pd.read_excel('../M0_IPPU/B1_Default_Param.xlsx')
    ...
    #
    for scen in range(len(scenario_list)):
        this_paramter_list_dir = '../M0_IPPU/A2_Output_Params/' + str(scenario_list[scen])
    ...
    with open('../M0_IPPU/check_stable_scenarios_freeze.pickle', 'wb') as handle:
    ...
    with open('../M0_IPPU/check_stable_scenarios.pickle', 'wb') as handle:
    ...
    if generator_or_executor == 'Generator' or generator_or_executor == 'Both':
        ...
        #
        print_adress = '../M0_IPPU/Executables'
        #
...
def main():
    ...
    print("*********************************************")
    print("1. Corriendo el modelo de del sector PIUP ...")
    print("**********************************************")
