...
def initial_code(file_table1, file_txt):
    #
...
def create_input_dataset_future_0_2(scenario_name, csv_params_scenario_path, output_dataset_path ):
...
def csv_generation():
    ...
    for ite in range(len(sce)):
        ...
        file_name = '../M2_AFOLU/2_Model/'+sce[ite]+'/ModeloSuelo_'+ sce[ite] +'.xlsx'
        ...
        for i in range(len(sheets_list)):
            ...
            df_param.to_csv( '../M2_AFOLU/1_Parameters/'+ sce[ite] + '/' + csv_name +'.csv', index = None, header=True)
            
        
            
        file_table1 = '../M2_AFOLU/0_Ref/STRUCTURE_OSEMOSYS_GUA_LAND.xlsx'
        file_txt = '../M2_AFOLU/0_Ref/data_land.txt'
        
        pre_file_name = '../M2_AFOLU/1_Parameters/' + sce[ite] + '/'
        
        
        
        header_indices = ['Scenario','Parameter','r','t','f','e','m','l','y','ls','ld','lh','s','value']
        S_DICT_sets_structure, S_DICT_params_structure, list_param_BAU, list_param_default_value_BAU = initial_code(file_table1, file_txt)
        base_year = 2016
        
        g= open( '../M2_AFOLU/2_Model/' + sce[ite] + '/data_land' + '.txt',"w+")
        g.write( '###############\n#    Sets     #\n###############\n#\n' )
        ...
        g.write('param DiscountRate default 0.12 :=\n;\n')
        ...
        create_input_dataset_future_0(sce[ite], '../M2_AFOLU/1_Parameters/'+sce[ite]+'/', '../M2_AFOLU/2_Model/'+sce[ite]+'/')
        
        create_input_dataset_future_0_2(sce[ite], '../M2_AFOLU/1_Parameters/'+sce[ite]+'/', '../M2_AFOLU/2_Model/'+sce[ite]+'/EXP/')
    
def main():
    print("*************************************************************************")
    print("5. Generando los archivos de datos de los escenarios del sector AFOLU ...")
    print("*************************************************************************")
    csv_generation()
    print("********************")
