...
def main_executer(n1, packaged_useful_elements, scenario_list_print, discount_year, discount_rate):
    ...
    file_aboslute_address = os.path.abspath("B1_Waste_EjecucionOSeMOSYS.py")
    file_adress = re.escape(file_aboslute_address.replace('B1_Waste_EjecucionOSeMOSYS.py', '')).replace('\:', ':').replace(r'Code\\', '')
    file_adress+=r'M1_Waste\\'
    #
...
def set_first_list(scenario_list_print):
    #
    first_list_raw = os.listdir('../M1_Waste/Executables')
    #
    global first_list
...
def data_processor(case, unpackaged_useful_elements, discount_year, discount_rate):
    ...
    #-------------------------------------------#
    structure_filename = "../M1_Waste/B1_Model_Structure.xlsx"
...
#        output_header.append(more_vars[v])
    ...
    data_name = str('../M1_Waste/Executables/' + first_list[case]) + '/' + str(first_list[case]) + '_output.txt'
    #
    n = 0
    ...
    #%%
    #-----------------------------------------------------------------------------------------------------------%
    output_adress = '../M1_Waste/Executables/' + str(first_list[case])
    ...
    for var in range(len(vars_as_appear)):
        ...
        for k in range(len(this_var_dict[this_variable])):
            ...
            if this_combination[2] in ['RM', 'FERT_ORG', 'salud_residuos', 'contam_agua', 'turismo_residuos','WTRU','HWT','CO2e','Accidents', 'Health', 'Congestion'] and this_variable == 'AnnualTechnologyEmissionPenaltyByEmission':
                ref_index = combination_list.index(this_combination)
                ...
                resulting_value = round(resulting_value_raw, 4) ### Probando con negativo para beneficios de residuos
                #
...
#            f.write('param ' + this_param + ':=\n')
    ...
    ###########################################################################################################################
    #
    with open('../M1_Waste/Executables' + '/' + str(scenario_list[scen]) + '_0' + '/' + str(scenario_list[scen]) + '_0' + '_Input.csv', 'w', newline = '') as param_csv:
...
def run_model(discount_year, discount_rate):
    ...
    # Year periods:
    dfyrs = \
        pd.read_excel('../M1_Waste/A1_Inputs/A-I_Horizon_Configuration.xlsx',
                      sheet_name='Horizon')
    ...
    *Abbreviations:*
    ...
    df_gdp_ref = pd.read_excel('../M1_Waste/_GDP_Ref.xlsx', 'GDP')
    ...
    #
    scenario_list_sheet = pd.read_excel('../M1_Waste/B1_Scenario_Config.xlsx', sheet_name='Scenarios')
    ...
    #sys.exit()
    #
    base_configuration_overall = pd.read_excel('../M1_Waste/B1_Scenario_Config.xlsx', sheet_name='Overall_Parameters')
    #
    all_dataset_address = '../M1_Waste/A2_Output_Params/'
    '''
    # Call the default parameters for later use:
    '''
    list_param_default_value = pd.read_excel('../M1_Waste/B1_Default_Param.xlsx')
    ...
    #
    for scen in range(len(scenario_list)):
        this_paramter_list_dir = '../M1_Waste/A2_Output_Params/' + str(scenario_list[scen])
    ...
    with open('../M1_Waste/check_stable_scenarios_freeze.pickle', 'wb') as handle:
    ...
    with open('../M1_Waste/check_stable_scenarios.pickle', 'wb') as handle:
    ...
    if generator_or_executor == 'Generator' or generator_or_executor == 'Both':
        ...
        #
        print_adress = '../M1_Waste/Executables'
        #
...
def main():
    ...
    print("**********************************************")
    print("3. Corriendo el modelo del sector Residuos ...")
    print("**********************************************")
