...
def set_txt_list( model_to_run, scenario ):
    #
    txt_list_raw = os.listdir( './2_Model/' + scenario + '/')
    #
...
def data_processor( case, model_to_run_gen, sce_gen ):
    ...
    structure_filename = "0_Ref/" + str( structural_list[ case ] )
    ...
    #
    data_name = './2_Model/' + sce_gen + '/' + str( case_name ) + '_output.txt'
    print( data_name )
    #
...
def main_executer( n1, sce ):
    #
    #set_txt_list( model_to_run, sce )
    #
...
for ite in range(len(sce)):
    ...
    set_txt_list( model_to_run, sce[ite] )
    
    for n1 in range( len(txt_list) ):
        main_executer(n1, sce[ite])