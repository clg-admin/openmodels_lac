...
def new_blend_2024(start_blend_point, mid_blend_point_one, final_blend_point, value_list, time_range_vector,name):
    name = name
    years = np.array([start_blend_point[0], mid_blend_point_one[0], final_blend_point[0]])
    values = np.array([start_blend_point[1] / 100, mid_blend_point_one[1] / 100, final_blend_point[1] / 100])
        ...
        if scenario_list[s] != 'BAU': # GENERALIZE THIS LATER.
            ...
            for n in range( len( Gasoline_Techs ) ):
                ...
                for n2 in range( len( Gasoline_Techs_Emissions[ Gasoline_Techs[n] ] ) ):
                    ...
                    final_blend_point = [2050, 10]


                    new_value_list_rounded, biofuel_shares = new_blend_2024(start_blend_point, mid_blend_point_one, final_blend_point, value_list, time_range_vector, Gasoline_Techs[n])
                    #
                    stable_scenarios[ scenario_list[ s ] ][ 'EmissionActivityRatio' ]['value'][ this_tech_emission_indices[0]:this_tech_emission_indices[-1]+1 ] = deepcopy( new_value_list_rounded )
                    #
