...
for g in range( len( groups_list ) ):
    ...
    for t in range( len( this_df_techs ) ):
        ...
        #
        #%% Test techs and techs inputs/ outputs
        # print(this_df_techs[t],output_fuel)
        # print(this_proj_df.loc[ ( this_proj_df[ 'Tech' ] == this_tech ) & ( this_proj_df[ 'Fuel' ] == output_fuel ) ])
        # print(this_proj_df_mode_o)
        # print(this_proj_df_param_o)
        # print(this_df_select_by_fo)
        # print(this_proj_df_local)
        #%%
        if groups_list[g] != 'Primary' and groups_list[g] != 'Transport':
...
for n in range( len( Projection_Driver_Vars ) ):
    ...
    if Projection_Mode_per_Driver[n] == 'Flat after final year':
        Projections_sheet[ this_col_header ].interpolate(method ='linear', limit_direction ='forward', inplace = True)
#
Projections_sheet.to_csv('./A2_Extra_Inputs/A-Xtra_Projections_COMPLETED.csv')
