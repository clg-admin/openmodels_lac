...
def main_executer(n1, packaged_useful_elements, scenario_list_print):
    print('See scenarios',n1, set_first_list(scenario_list_print))
    set_first_list(scenario_list_print)
    print("Lets run data_procesor")
    # print(scenario_list_print)
...
def interpolation_blend( start_blend_point, mid_blend_point, final_blend_point, value_list, time_range_vector ):
    ...
    #
#TODO 5/6 Added new interpolation function
def new_blend_2024(start_blend_point, mid_blend_point_one, mid_blend_point_two,mid_blend_point_three, final_blend_point, value_list, time_range_vector,name):
    name = name
    years = np.array([start_blend_point[0], mid_blend_point_one[0], mid_blend_point_two[0],
                      mid_blend_point_three[0] , final_blend_point[0]])
    values = np.array([start_blend_point[1] / 100, mid_blend_point_one[1] / 100, mid_blend_point_two[1] / 100,
                       mid_blend_point_three[1] / 100, final_blend_point[1] / 100])
...
if __name__ == '__main__':
    ...
    *Abbreviations:*
    ...
    for n in range( len( base_configuration_modeshift.index ) ):
        ...
        #
    #
    # print(base_configuration_modeshift)
        ...
        if scenario_list[s] != 'BAU': # GENERALIZE THIS LATER.
            ...
            for n in range( len( Diesel_Techs ) ):
                ...
                for n2 in range( len( Diesel_Techs_Emissions[ Diesel_Techs[n] ] ) ):
                    ...
                    value_list = [ float(value_list[j]) for j in range( len(value_list) ) ]
                    #
                    start_blend_point = [2026, 0]
                    final_blend_point = [2030, 0]
            ...
            for n in range( len( Gasoline_Techs ) ):
                ...
                for n2 in range( len( Gasoline_Techs_Emissions[ Gasoline_Techs[n] ] ) ):
                    ...
                    mid_blend_point_two = [2030, 12]
                    mid_blend_point_three = [2040, 15]
                    final_blend_point = [2050, 20]


                    new_value_list_rounded, biofuel_shares = new_blend_2024(start_blend_point, mid_blend_point_one, mid_blend_point_two, mid_blend_point_three, final_blend_point, value_list, time_range_vector,Gasoline_Techs[n])
                    #
                    stable_scenarios[ scenario_list[ s ] ][ 'EmissionActivityRatio' ]['value'][ this_tech_emission_indices[0]:this_tech_emission_indices[-1]+1 ] = deepcopy( new_value_list_rounded )
                    #
    ...
    if is_this_last_update == True:
        ...
        x = len(scenario_list)
        for n in range(len(scenario_list)):
            csv_printer_parallel(n, scenario_list, stable_scenarios, basic_header_elements, parameters_to_print,
    ...
    if generator_or_executor == 'Generator' or generator_or_executor == 'Both':
        ...
        # Parallel
        num_processes = 2  # Set to 2 processes
        with mp.Pool(processes=num_processes) as pool:
            # Distribute the tasks among the processes
            pool.starmap(function_C_mathprog,
                         [(n, stable_scenarios, packaged_useful_elements)
                          for n in range(len(scenario_list))])

        # One by one
        # for n in range(len(scenario_list)):
        #     function_C_mathprog(n, stable_scenarios, packaged_useful_elements)
...
#%%
    ...
    if generator_or_executor == 'Executor' or generator_or_executor == 'Both':
        ...
        with mp.Pool(processes=num_processes,maxtasksperchild=1) as pool:
            pool.starmap(Modify_m1_Escenario.run_model, [(scenario,) for scenario in scenario_list_print])


        # One solution at time
        # print("Lets run OSeMOSYS Models one at time")
        # # Modify_m1_Escenario.run_model('BAU')
        # for scenario in scenario_list_print:
        #     print(scenario)
        #     Modify_m1_Escenario.run_model(scenario)

# Parallel
        num_processes = 2  # Set to 2 processes
        with mp.Pool(processes=num_processes) as pool:
            # Distribute the tasks among the processes
            pool.starmap(main_executer,
                         [(n, packaged_useful_elements, scenario_list_print)
                          for n in range(len(first_list))])
# Choose All scenarios
#         for n in range(len(first_list)):
#             main_executer(n, packaged_useful_elements, scenario_list_print)
