    ...
    for s in range( len( scenario_list ) ):
            ...
            #
                ...
                for a_set in range( len( applicable_sets_demand ) ): # FLAG: in the future we must consider non-logistic behaviors
                    ...
                    if modeshift_params[ scenario_list[s] ][ this_set ][ params['type'] ] == params['logistic']:
                       ...
                       # shift_years = [ n for n in range( Initial_Year_of_Uncertainty,params['final_year']+1 ) ]
        ...
        if params['Use_Transport_B1']:
            ...
            for t in range(len(rewrite_techs_lowlim)):
                ...
                # if this_set == 'TRNFRELIGGSL' and scenario_list[ s ] == "BAU":
                #     sys.exit(5)
                ...
                if scenario_list[s] != params['BAU']:
                    ...
                    for cp in range( len( cap_vars ) ):
                        ...
                        for t in range( len( all_sets ) ):
                           ...
                           if ((all_sets[t] in applicable_sets or 
                                (any(all_sets[t].endswith(suffix) for suffix in params['suffixes_fuel']) and params['power_plants'][0] in all_sets[t]) or 
                                (not any(all_sets[t].endswith(suffix) for suffix in params['suffixes_fuel']) and params['tr'] not in all_sets[t]) or
                                (not any(all_sets[t].endswith(suffix) for suffix in params['suffixes_fuel']) and params['tr'] in all_sets[t])) or
            ...
            for l in range( len( set_list ) ):
                ...
                for p in range( len( param_list ) ):
                    ...
                    if electrical_Params_built_in == 'YES':
                        ...
                        # if set_list[l] == "PWRFOI" and param_list[p] == "TotalAnnualMaxCapacity":
                        #     print("here!")
                        #     sys.exit(5)
                ...
                for p in range( len( param_list ) ):
                    ...
                    for emis in emission_list:   
                        if type(emis)==str: 
                            ...
                            this_scenario_tech_param_df = this_scenario_tech_df.loc[(this_scenario_tech_df['Parameter'] == param_list[p])]                                                                                         
                        # if param_list[p] == "InputActivityRatio" and math.isnan(emis) :
                        #     sys.exit(5)
                        #
                        if len(this_scenario_tech_param_df.index.tolist()) > 0:
                            ...
                            if smartGrid_Params_built_in == 'YES':
                                if (( isinstance(emis, float) and not math.isnan(emis)    # es float y no es NaN
                                     ) or ( isinstance(emis, str) and emis != ''               # es str y no está vacío
                                           )) and param_list[p] == 'EmissionActivityRatio':
