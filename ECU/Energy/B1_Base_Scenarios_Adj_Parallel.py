...
'''
We implement OSEMOSYS-ECU-NDC in a csv based system for semi-automatic manipulation of parameters.
'''
                ...
                if this_tech in list( Fleet_Groups_inv.keys() ):
                    ...
                    if 'Trains' not in group_tech and 'Telef' not in group_tech:
                        ...
                        driven_distance_2024 = \
                            float(Reference_driven_distance[this_strategy][group_tech][time_range_vector.index(INFLEX_YEAR)])
                        driven_distance_final = \
                            float(Reference_driven_distance[this_strategy][group_tech][time_range_vector.index(END_YR)])
                        ...
                        if this_variable == 'NewCapacity':
                            ...
                            if int(this_year) >= INFLEX_YEAR and driven_distance_2024 != driven_distance_final:
                                # We need to select the appropiate accumulated new capacity:
                                accum_new_cap = \
                                    aide_dict_accumnewcap[this_tech][this_year_index-1]

                                if int(this_year) > INI_YR+Reference_op_life[this_strategy][this_tech]:  # works with a timeframe by 2050; longer timeframes may need more lifetime cycle adjustments
                                    subtract_index_accum_old = \
                                        int(Reference_op_life[this_strategy][this_tech])
                                    if int(this_year) - subtract_index_accum_old <= INFLEX_YEAR:
                                        driven_distance_17 = \
                                            float(Reference_driven_distance[this_strategy][group_tech][time_range_vector.index(INI_YR)])
                                    else:
                                        driven_distance_17 = \
                                    ...
                                    if int(this_year) - subtract_index_accum_old > INFLEX_YEAR:  # this is an odd rule, but works empirically
                                        driven_distance_2024 = \
                                            float(Reference_driven_distance[this_strategy][group_tech][time_range_vector.index(INFLEX_YEAR)])
            ...
            if this_variable == 'CapitalInvestment':
                ...
                if this_tech in list(Fleet_Groups_inv.keys()):
                    ...
                    if 'Trains' not in group_tech and 'Telef' not in group_tech:
                        ...
                        accum_new_cap_subtract = 0
                        driven_distance_prev = float(Reference_driven_distance[this_strategy][group_tech][this_year_index-1])
                        if int(this_year) >= INFLEX_YEAR and driven_distance_2024 != driven_distance_final:  # greater than the base year
                            # We need to select the appropiate accumulated new capacity:
                            accum_new_cap = \
                                aide_dict_accumnewcap[this_tech][this_year_index-1]

                            if int(this_year) > INI_YR+Reference_op_life[this_strategy][this_tech]:
                                subtract_index_accum_old = \
                                    int(Reference_op_life[this_strategy][this_tech])
                                if int(this_year) - subtract_index_accum_old <= INFLEX_YEAR:
                                    driven_distance_17 = \
                    ...
                    for nx in range(len(concat_result_set)):
                        ...
                        for s in range(len(second_last_set_element_unique)):
                            ...
                            if len(value_indices) != 0:
                                ...
                                if len(value_indices) == 0:
                                    print(this_param)
                                    print(second_last_set_element_unique[s])
                                    print(this_set_element_unique_1[n1])
                                    print(n1_faster)
                                    print(n2_faster)
                                    sys.exit()
                                ...
                                for val in range(len(value_indices)):
                                    ...
                                    try:
                                        ...
                                        print(value_indices)
                                        sys.exit()
                                        # print(this_scenario_data[this_param]['value'])
                                    '''
...
if __name__ == '__main__':
    #
    all_years = [ y for y in range( INI_YR , END_YR+1 ) ]
    index_2024 = all_years.index( INFLEX_YEAR )
    ...
    *Abbreviations:*
    ...
    for s in range( len( scenario_list ) ):
        ...
        #             #
        #             known_years_ini = [e for e in range(INI_YR, int(y_ini)+1)]
            ...
            for p in range(len(parameters_involved)):
                ...
                for a_set in range( len( transport_group_sets ) ):
                    ...
                    try:
                        ...
                        new_distance = deepcopy(base_distance)
            ...
            for t in range( len( tech_list ) ):
                ...
                for p in range( len( param_list ) ):
                    ...
                    if smartGrid_Params_built_in == 'YES':
                        if param_list[p] != 'EmissionActivityRatio':
                            this_set = this_set_raw
                            this_param_indices = [ i for i, x in enumerate( stable_scenarios[ this_scenario_name ][ param_list[p] ][ 't' ] ) if x == str( this_set ) ]
                            
                            # print('are we good here? - non EAR')
                            # sys.exit()
                            
                        else:
                            this_set = this_set_raw.split(' ; ')[0]
                            this_emis = this_set_raw.split(' ; ')[-1]
                            this_param_indices_set = [ i for i, x in enumerate( stable_scenarios[ this_scenario_name ][ param_list[p] ][ 't' ] ) if x == str( this_set ) ]
                            this_param_indices_emis = [ i for i, x in enumerate( stable_scenarios[ this_scenario_name ][ param_list[p] ][ 'e' ] ) if x == str( this_emis ) ]

                            this_param_indices = list(set(this_param_indices_set) & set(this_param_indices_emis))
                            this_param_indices.sort()

                            # print('are we good here? - YES ear', this_set_raw)
                            # sys.exit()
                    ...
                    if smartGrid_Params_built_in == 'YES':
                        ...
                        #if 'PPRNGS ; CO2e_PP' == this_set_raw:
                        #    print('what are we checking here')
                        #    sys.exit()
                        stable_scenarios[ this_scenario_name ][ param_list[p] ]['value'][ this_param_indices[0]:this_param_indices[-1]+1 ] = deepcopy( value_list )
